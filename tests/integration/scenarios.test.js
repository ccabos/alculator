 ...
 * Python reference peaks (scripts/generate_curves.py, r = 0.789, Km = 0.005 %):
 *   Ex 1: 0.0451 %   Ex 2: 0.0293 %   Ex 3: 0.0284 %
 *   Ex 4: 0.0333 %   Ex 5: 0.0448 %   Ex 6: 0.0309 %
 */
...
describe('Scenario 1 — Three wines, fasted (baseline)', () => {
  it('peak BAC matches Python reference (0.0451 %)', () => {
    const drinks = [
      wine(1140), // 19:00
      wine(1200), // 20:00
      wine(1260), // 21:00
    ];
    expect(peakBAC(drinks, [])).toBeCloseTo(0.0451, 3); // 3 decimal places → ±0.0005
  });
});
...
describe('Scenario 2 — Heavy meal at 18:30, drinks at 19:00/20:00/21:00', () => {
  it('peak BAC matches Python reference (0.0293 %)', () => {
    const drinks = [wine(1140), wine(1200), wine(1260)];
    const food   = [{ time_min: 1110, type: 'heavy_meal' }];
    expect(peakBAC(drinks, food)).toBeCloseTo(0.0293, 3);
  });
});
...
describe('Scenario 3 — Wine 1 (19:00) fully absorbed before meal (20:00): not covered', () => {
  it('peak BAC matches Python reference (0.0284 %)', () => {
    const drinks = [
    ...
    expect(peakBAC(drinks, food)).toBeCloseTo(0.0284, 3);
  });
});
...
describe('Scenario 4 — Wine 1 (19:30) still absorbing when meal arrives (20:00): Case B', () => {
  it('peak BAC matches Python reference (0.0333 %)', () => {
    const drinks = [
    ...
    const food = [{ time_min: 1200, type: 'full_meal' }];
    expect(peakBAC(drinks, food)).toBeCloseTo(0.0333, 3);
  });
});
...
describe('Scenario 5 — Snack at 19:30: Wines 1 & 2 covered, Wine 3 fasted', () => {
  it('peak BAC matches Python reference (0.0448 %)', () => {
    const drinks = [
    ...
    expect(peakBAC(drinks, food)).toBeCloseTo(0.0448, 3);
  });
});
...
describe('Scenario 6 — Champagne (19:00) + wines at dinner, full_meal at 18:30', () => {
  it('peak BAC matches Python reference (0.0309 %)', () => {
    const drinks = [
    ...
    expect(peakBAC(drinks, food)).toBeCloseTo(0.0309, 3);
  });
});
